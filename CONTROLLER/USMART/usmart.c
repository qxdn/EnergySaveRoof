...
void Timer6_Init(u16 arr, u16 psc)
{
    ...
    RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM6, ENABLE); ///使能TIM1时钟,总线2
    ...
    TIM_TimeBaseInit(TIM6, &TIM_TimeBaseInitStructure); //初始化定时器1

    TIM_ClearFlag(TIM6, TIM_FLAG_Update);
    TIM_ITConfig(TIM6, TIM_IT_Update, ENABLE); //允许定时器1更新中断
    TIM_Cmd(TIM6, ENABLE);                     //使能定时器1

    NVIC_InitStructure.NVIC_IRQChannel = TIM6_DAC_IRQn;          //外部中断4
