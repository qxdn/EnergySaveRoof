...
void OLED_Init()
{
	...
	GPIO_Init(GPIOB,&GPIO_InitStructure); //初始化结构体  PB0 PB1
 	GPIO_SetBits(GPIOB,GPIO_Pin_0|GPIO_Pin_1);	//拉高电平
	GPIO_InitStructure.GPIO_Pin=GPIO_Pin_7|GPIO_Pin_8|GPIO_Pin_10;  //PE7 PE8 PE10
	GPIO_Init(GPIOE,&GPIO_InitStructure); //初始化结构体 
 	GPIO_SetBits(GPIOE,GPIO_Pin_7|GPIO_Pin_8|GPIO_Pin_10);	//拉高电平
#endif 	
	...
	GPIO_InitStructure.GPIO_Mode=GPIO_Mode_OUT; //输出模式
	GPIO_InitStructure.GPIO_Pin=GPIO_Pin_3|GPIO_Pin_4|GPIO_Pin_5|GPIO_Pin_6|GPIO_Pin_7;//管脚设置
	GPIO_InitStructure.GPIO_Speed=GPIO_Speed_100MHz;//速度为100M
	GPIO_InitStructure.GPIO_OType=GPIO_OType_PP;//推挽输出
	GPIO_InitStructure.GPIO_PuPd=GPIO_PuPd_UP;//上拉
...
{
	char string[10];
	sprintf(string,"%5.2f",val);
	OLED_ShowString(x,y,(u8 *)string,12);
}
